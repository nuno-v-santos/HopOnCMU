...
<#switch relation.type>
   ...
   /* public List<${relation.target.name}> get${relation.target.name}s() {
    ...
    public void set${relation.target.name}(${relation.target.name} ${relation.target.name?lower_case}) {
        ...
        String query = String.format("Update %s set ${relation.target.name?lower_case}_id = '%d'  where id = %d", "${relation.base.name}", ${relation.target.name?lower_case}.getId(), this.id);
    ...
    public void set${relation.base.name}(${relation.base.name} ${relation.base.name?lower_case}) {
        ...
        String query = String.format("Update %s set ${relation.target.name?lower_case}_id = '%d'  where id = %d", "${relation.base.name}",this.id, ${relation.base.name?lower_case}.getId());
        con.executeUpdate(query);
